...
export class SignalGenerator {
  ...
  constructor() {
    ...
    if (perplexityKey) {
      ...
      throw new Error('No LLM API key found. Set PERPLEXITY_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY');
    }
  }
  ...
  async generateSignal(input: SignalGeneratorInput): Promise<TradingSignal> {
    console.log(`[SignalGen] Generating signal for ${input.tokenSymbol} on ${input.venue}`);
...
${input.marketIndicators.currentPrice ? `- Current Price: $${input.marketIndicators.currentPrice}` : ''}
${input.marketIndicators.rsi ? `- RSI (14): ${input.marketIndicators.rsi.toFixed(2)} ${this.interpretRSI(input.marketIndicators.rsi)}` : ''}
${input.marketIndicators.macd ? `- MACD: ${input.marketIndicators.macd.histogram.toFixed(2)} ${this.interpretMACD(input.marketIndicators.macd)}` : ''}
${input.marketIndicators.priceChange24h ? `- 24h Change: ${input.marketIndicators.priceChange24h.toFixed(2)}%` : ''}
${input.marketIndicators.volume24h ? `- 24h Volume: $${(input.marketIndicators.volume24h / 1e6).toFixed(2)}M` : ''}
...
Respond ONLY with the JSON object, no other text.`;
   ...
   * Parse LLM response
   */
  private parseResponse(response: string, input: SignalGeneratorInput): TradingSignal {
    try {
  ...
  private fallbackSignal(input: SignalGeneratorInput): TradingSignal {
    ...
    if (input.marketIndicators?.rsi) {
      ...
      if (side === 'SHORT' && input.marketIndicators.rsi > 60) confidence *= 1.1;
      if (side === 'LONG' && input.marketIndicators.rsi > 70) confidence *= 0.8;
      if (side === 'SHORT' && input.marketIndicators.rsi < 30) confidence *= 0.8;
    }
    ...
    return {
      ...
      leverage,
      reasoning: `Rule-based signal: ${input.tweetSentiment} sentiment with ${(confidence * 100).toFixed(0)}% confidence based on tweet analysis${input.marketIndicators ? ' and market indicators' : ''}.`,
      riskLevel,
    };
  }
