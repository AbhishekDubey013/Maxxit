...
async function testBothSafes() {
  try {
    ...
    console.log('🧪 TESTING BOTH SAFES: 1 USDC → WETH\n');
    ...
    console.log('Test 1 (Your Safe):', result1.success ? '✅ PASSED' : '❌ FAILED');
    if (!result1.success) {
      console.log('  Error:', result1.error?.substring(0, 100) + '...');
    }
    console.log('');
    
    console.log('Test 2 (Original Safe):', result2.success ? '✅ PASSED' : '❌ FAILED');
    if (!result2.success) {
    ...
    if (result1.success && !result2.success) {
      console.log('   Your Safe works, Original Safe failed.');
      console.log('   → Likely gas issue (executor ran out after 1st trade)');
    } else if (!result1.success && !result2.success) {
      console.log('   Both failed.');
      console.log('   → Executor wallet has insufficient gas for ANY trade');
    } else if (result1.success && result2.success) {
      console.log('   Both worked! 🎉');
      console.log('   → No gas issue, executor wallet has enough ETH');
    }
