...
logger.info("=" * 60)
encryption_key = os.getenv('AGENT_WALLET_ENCRYPTION_KEY')
platform_key = os.getenv('PLATFORM_MASTER_KEY')

if encryption_key:
    logger.info(f"✅ AGENT_WALLET_ENCRYPTION_KEY: Set")
    ...
    logger.error("❌ AGENT_WALLET_ENCRYPTION_KEY: NOT SET")
...
def require_encryption_key():
    ...
    key_hex = os.getenv('AGENT_WALLET_ENCRYPTION_KEY')
    if not key_hex:
        raise Exception('AGENT_WALLET_ENCRYPTION_KEY not configured')
    if len(key_hex) != 64:
        raise Exception('AGENT_WALLET_ENCRYPTION_KEY must be a 32-byte hex string (64 chars)')
...
def decrypt_private_key(cipher_text: str, iv: str, tag: str):
    ...
    try:
        ...
        try:
            ...
            logger.error(f'[OstiumService] Base64 decode failed: {str(decode_error)}')
    ...
    try:
        ...
        if agent_address:
            try:
                ...
                # Query user_ostium_wallets for agent's encrypted private key
                conn = psycopg2.connect(database_url)
                cur = conn.cursor(cursor_factory=RealDictCursor)
                cur.execute(
                    """
                    SELECT agent_private_key_encrypted, agent_key_iv, agent_key_tag 
                    FROM user_ostium_wallets 
                    WHERE LOWER(agent_address) = LOWER(%s)
                    """,
                    (agent_address,)
                )
                wallet = cur.fetchone()
                ...
                if not wallet:
                    return jsonify({
                        "success": False,
                        "error": f"Agent address {agent_address} not found in user_ostium_wallets"
                    }), 404
                
                # Decrypt the private key
                try:
                    logger.info(f"[Decrypt] Attempting to decrypt key for agent: {agent_address}")
                    logger.info(f"[Decrypt] Encrypted data lengths - cipher: {len(wallet['agent_private_key_encrypted'])}, iv: {len(wallet['agent_key_iv'])}, tag: {len(wallet['agent_key_tag'])}")
                    logger.info(f"[Decrypt] Cipher text preview: {wallet['agent_private_key_encrypted'][:20]}...")
                    
                    private_key = decrypt_private_key(
                        wallet['agent_private_key_encrypted'],
                        wallet['agent_key_iv'],
                        wallet['agent_key_tag']
                    )
                    use_delegation = True
