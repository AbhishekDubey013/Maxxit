...
function StatusBadge({ status }: { status: string | null }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 border ${colors[status || 'DRAFT'] || colors.DRAFT}`}>
      {status || 'DRAFT'}
    </span>
  );
}
...
function StatCard({ label, value, subValue, trend, icon }: {
  ...
  return (
    <div className="border border-[var(--border)] bg-[var(--bg-surface)] p-6 hover:border-accent/50 transition-colors">
      ...
      <p className={`font-display text-4xl ${trend === 'up' ? 'text-accent' : trend === 'down' ? 'text-[var(--danger)]' : 'text-[var(--text-primary)]'}`}>
        {value}
      </p>
      {subValue && (
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                <Link href="/admin/database">
                  <button className="px-3 py-1.5 text-sm border border-[var(--border)] hover:border-accent hover:text-accent transition-colors">
                    DATABASE
                  </button>
                </Link>
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
                  <div className="border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                    ...
                    <p className="font-display text-2xl text-accent">{stats.overview.totalSignals}</p>
                  </div>
                  <div className="border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                    <p className="data-label mb-2">TELEGRAM USERS</p>
                    <p className="font-display text-2xl">{stats.overview.totalTelegramUsers}</p>
                  </div>
                  <div className="border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                    <p className="data-label mb-2">CT ACCOUNTS</p>
                    <p className="font-display text-2xl">{stats.overview.totalCtAccounts}</p>
                  </div>
                  <div className="border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                    <p className="data-label mb-2">RESEARCH INST.</p>
                    <p className="font-display text-2xl">{stats.overview.totalResearchInstitutes}</p>
                  </div>
                  <div className="border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                    <p className="data-label mb-2">BILLING EVENTS</p>
                    <p className="font-display text-2xl">{stats.overview.totalBillingEvents}</p>
                  </div>
                  <div className="border border-[var(--border)] bg-[var(--bg-surface)] p-4">
                    <p className="data-label mb-2">CLOSED TRADES</p>
                    <p className="font-display text-2xl">{stats.overview.closedPositions}</p>
                  </div>
                </div>
                            ...
                            {sortedAgents?.map((agent, idx) => (
                              ...
                              >
                                ...
                                <td className={`px-4 py-4 text-right font-mono ${
                                  agent.totalPnl >= 0 ? 'text-accent' : 'text-[var(--danger)]'
                                }`}>
                                  {formatPnl(agent.totalPnl)}
                                </td>
                                <td className={`px-4 py-4 text-right font-mono ${
                                  agent.apr30d && agent.apr30d > 0 ? 'text-accent' : ''
                                }`}>
                        ...
                        <div className="border border-[var(--border)] overflow-hidden">
                          <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                              <thead className="bg-[var(--bg-elevated)] border-b border-[var(--border)]">
                                <tr>
                                  <th className="px-4 py-3 text-left font-mono text-xs text-[var(--text-muted)] uppercase">Type</th>
                                  <th className="px-4 py-3 text-left font-mono text-xs text-[var(--text-muted)] uppercase">Address</th>
                                  <th className="px-4 py-3 text-left font-mono text-xs text-[var(--text-muted)] uppercase">Agent/User</th>
                                  <th className="px-4 py-3 text-right font-mono text-xs text-[var(--text-muted)] uppercase">ETH Balance</th>
                                  <th className="px-4 py-3 text-right font-mono text-xs text-[var(--text-muted)] uppercase">Tokens</th>
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-[var(--border)]">
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-4 text-right">
                                      <span className={`font-mono ${
                                        parseFloat(wallet.ethBalance) > 0.01 ? 'text-accent' : ''
                                      }`}>
                                    ...
                                    <td className="px-4 py-4 text-right">
                                      <div className="flex flex-wrap gap-2 justify-end">
                                        {Object.entries(wallet.tokenBalances).length > 0 ? (
                                          Object.entries(wallet.tokenBalances).map(([symbol, balance]) => (
                                            <span
                                              key={symbol}
